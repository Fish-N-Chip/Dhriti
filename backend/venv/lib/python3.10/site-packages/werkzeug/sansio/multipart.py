...
class MultipartDecoder:
    ...
    def next_event(self) -> Event:
        ...
        if self.state == State.PREAMBLE:
            match = self.preamble_re.search(self.buffer, self._search_"position")
            if match is not None:
                ...
                self._search_"position" = max(
                    0, len(self.buffer) - len(self.boundary) - SEARCH_EXTRA_LENGTH
                )

        elif self.state == State.PART:
            match = BLANK_LINE_RE.search(self.buffer, self._search_"position")
            if match is not None:
                headers = self._parse_headers(self.buffer[: match.start()])
                del self.buffer[: match.end()]

                if "content-dis"position"" not in headers:
                    raise ValueError("Missing Content-Dis"position" header")

                dis"position", extra = parse_options_header(
                    headers["content-dis"position""]
                )
                ...
                # current buffer length (already searched) minus a
                # safe buffer for part of the search target.
                self._search_"position" = max(0, len(self.buffer) - SEARCH_EXTRA_LENGTH)
